...
// All snapshot and summary types used by normalize functions are defined in index.ts.
// We import them as types only (erased at runtime) to avoid circular dependency issues.
import type {
  MlcReadSnapshotKind, MlcReadSnapshotCompleteness, MlcReadSnapshotConfidence,
  MlcReadSnapshotFreshness, MlcReadSnapshot,
  MlcListingSummary, MlcListingsSnapshot,
  MlcOrderSummary, MlcOrdersSnapshot,
  MlcMessageSummary, MlcMessagesSnapshot,
  MlcQuestionSummary, MlcQuestionsSearchResult, MlcQuestionsSnapshot,
  MlcReputationSummary, MlcReputationSnapshot,
  MlcVisitsDetail, MlcVisitsSummary, MlcVisitsSnapshot,
  MlcVisitsTimeWindowSummary, MlcVisitsTimeWindowSnapshot,
  MlcPerformanceRule, MlcPerformanceVariable, MlcPerformanceBucket,
  MlcPerformanceSummary, MlcPerformanceSnapshot,
  MlcRelistResult, MlcRelistSnapshot,
  MlcImageDetection, MlcImageDiagnosticResult, MlcImageDiagnosticSummary,
  MlcImageDiagnosticSnapshot, MlcImageUploadResult, MlcImageUploadSnapshot,
  MlcImageOrchestrationStep, MlcImageOrchestrationSummary,
  MlcModerationStatusSummary, MlcModerationStatusSnapshot,
  MlcNoticesSummary, MlcNoticesSnapshot,
  MlcAnswerSummary, MlcAnswerSnapshot,
  MlcClaimPlayer, MlcClaimPlayerAction, MlcClaimResolution, MlcClaimMessage,
  MlcClaimSummary, MlcClaimsSearchResult, MlcClaimsSearchSnapshot,
  MlcClaimDetailSummary, MlcClaimDetailSnapshot,
  MlcClaimMessagesSummary, MlcClaimMessagesSnapshot,
  MlcClaimResolutionsSummary, MlcClaimResolutionsSnapshot,
  MlcClaimReputationSummary, MlcClaimReputationSnapshot,
  MlcClaimStatusHistorySummary, MlcClaimStatusHistorySnapshot,
  MlcRateLimitBlockedMetadata,
  MlcShipmentStatusSummary, MlcShipmentStatusSnapshot,
  MlcReturnSnapshotBase, MlcReturnSummary, MlcClaimReturnSummary,
  MlcClaimReturnSnapshot, MlcReturnReview, MlcReturnReviewsSummary,
  MlcReturnReviewsSnapshot, MlcReturnCostCharge, MlcClaimReturnCostSummary,
  MlcClaimReturnCostSnapshot,
  MlcCategoryAttributeSummary, MlcCategoryAttributesSnapshot,
  MlcCategoryTechnicalSpecSummary, MlcCategoryTechnicalSpecsSnapshot,
  MlcProductAdsMetricSummary, MlcProductAdsEntitySummary,
  MlcProductAdsInsights, MlcProductAdsInsightsSnapshot,
  MlcProductAdsInsightsOptions,
  MlcListingPricesInput, MlcSaleFeeDetails, MlcListingPriceSummary,
  MlcListingPricesSnapshot,
  MlcItemSalePriceSummary, MlcItemSalePriceSnapshot,
  MlcItemPriceConditionsSummary, MlcItemPricesSummary, MlcItemPricesSnapshot,
  MlcPriceToWinWinnerSummary, MlcPriceToWinBoostSummary,
  MlcPriceToWinSummary, MlcPriceToWinSnapshot,
  MlcPricingAutomationSummary, MlcPricingAutomationSnapshot,
  MlcPricingAutomationRulesSummary, MlcPricingAutomationRulesSnapshot,
  MlcPricingAutomationHistorySummary, MlcPricingAutomationHistorySnapshot,
  MlcAutomatedPriceItemsSummary, MlcAutomatedPriceItemsSnapshot,
  MlcPromotionBenefitsSummary, MlcPromotionSummary,
  MlcPromotionParticipantSummary, MlcItemPromotionStockSummary,
  MlcItemPromotionSummary, MlcSellerPromotionsSummary,
  MlcSellerPromotionsSnapshot, MlcPromotionDetailSnapshot,
  MlcPromotionItemsSummary, MlcPromotionItemsSnapshot,
  MlcItemPromotionsSnapshot, MlcSingleReadSnapshot,
  MlcCategoryIdentifierFailure,
} from "./index.js";
...
export function pushOptional<T extends object, K extends keyof T>(
  target: T,
  key: K,
  value: T[K] | undefined,
): void {
...
export function assertMlcItemId(itemId: string): string {
...
// ---------------------------------------------------------------------------
// Normalize functions — Listing/Order/Message family
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Normalize — Image diagnostic / upload
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Normalize — Category attributes / technical specs
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Normalize — Item sale price / prices / price to win
// ---------------------------------------------------------------------------
...
function normalizePromotionSummary(raw: unknown): MlcPromotionSummary | undefined {
  ...
  if (rawOffers.length > 0) {
    type OffersType = MlcPromotionSummary["offers"] extends ReadonlyArray<infer T> | undefined
      ? T[]
      : never;
...
function normalizeSellerPromotions(input: {
  ...
  const paging: MlcSellerPromotionsSummary["paging"] = {
    offset: numberValue(pagingRecord?.offset) ?? 0,
    limit: numberValue(pagingRecord?.limit) ?? 50,
  };
  pushOptional(paging, "total", numberValue(pagingRecord?.total));
...
// ---------------------------------------------------------------------------
// Normalize — Items (write operations), Categories, Users
// ---------------------------------------------------------------------------
...
export {
  // Helpers
  createFreshness,
  // Assertion functions (inline-exported omitted: assertMlcItemId, assertCompleteMlcItem, normalizeMlcItemId, normalizeImageOrchestration)
  assertMlcCategoryId,
  ...
  productAdsQuery,
  listingPricesQuery,
  // Normalize functions — all internal
  normalizeListings,
  normalizeOrders,
  normalizeMessages,
  normalizeReputation,
