...
describe("Economic Durability Migration", () => {
  ...
  describe("v1 → v5 upgrade", () => {
    it("creates all new columns and tables (v2-v5)", () => {
      ...
      const ccCols = db.prepare(
        "PRAGMA table_info(economic_cost_components)",
      ).all() as Array<{ name: string }>;
      const ccColNames = ccCols.map((c) => c.name);
      expect(ccColNames).toContain("ingestion_run_id");

      // Verify v4: ingestion_run_id on snapshots
      const snapCols = db.prepare(
        "PRAGMA table_info(unit_economics_snapshots)",
      ).all() as Array<{ name: string }>;
    ...
    it("preserves existing data through migration", () => {
      ...
      const run = db.prepare(
        "SELECT id, seller_id, status FROM economic_ingestion_runs WHERE id = ?",
      ...
      const cc = db.prepare(
        "SELECT id, seller_id, type FROM economic_cost_components WHERE id = ?",
      ...
      const ccNewCol = db.prepare(
        "SELECT ingestion_run_id FROM economic_cost_components WHERE id = ?",
  ...
  describe("evidence table creation", () => {
    it("creates evidence table with correct structure", () => {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?)
      `).run(
        "ev-test-1", "plasticov", "mercadolibre", "order", "order-001",
        null, Date.now(), Date.now() - 86400000, "2026-01-01", "sha256:abc",
        "verified", 0.95, "run-001", Date.now(),
      );
