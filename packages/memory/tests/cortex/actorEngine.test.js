...
describe("seedActorNodes", () => {
    ...
    it("is idempotent — re-seeding updates, does not duplicate", () => {
        ...
        const count = db.prepare("SELECT COUNT(*) as cnt FROM nodes").get().cnt;
        expect(count).toBe(3);
    });
});
describe("getActorNode", () => {
    ...
    it("returns null for an invalid actor type", () => {
        const node = engine.getActorNode("unknown");
...
describe("reinforceActorOutcome", () => {
    ...
    it("is a no-op for unknown actor types", () => {
        expect(() => engine.reinforceActorOutcome("unknown", true)).not.toThrow();
    });
});
...
//# sourceMappingURL=actorEngine.test.js.map