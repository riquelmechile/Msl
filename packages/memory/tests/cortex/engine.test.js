...
describe("GraphEngine", () => {
    ...
    describe("spreadActivation", () => {
        ...
        it("sorts activated nodes by activation descending", () => {
            ...
            for (let i = 1; i < result.activatedNodes.length; i++) {
                expect(result.activatedNodes[i - 1].activation).toBeGreaterThanOrEqual(result.activatedNodes[i].activation);
            }
        });
    ...
    describe("prune", () => {
        ...
        it("is idempotent — re-run returns 0 after pruning", () => {
            ...
            const count = db.prepare("SELECT COUNT(*) as cnt FROM darwinian_lessons").get().cnt;
            expect(count).toBe(1);
        });
        it("uses distilled_lesson when present on the edge", () => {
            ...
            const b = engine.createNode("B");
            const edge = engine.createEdge(a.id, b.id);
            db.prepare("UPDATE edges SET weight = 0.02, distilled_lesson = ? WHERE id = ?").run("custom distilled insight", edge.id);
            engine.prune();
            const lesson = db
        ...
        it("is atomic — both INSERT and DELETE happen or neither does", () => {
            ...
            const lessonCount = db.prepare("SELECT COUNT(*) as cnt FROM darwinian_lessons").get().cnt;
    ...
    describe("traverse", () => {
        ...
        it("returns traversed edges with weights and co-occurrence counts", () => {
            ...
            db.prepare("UPDATE edges SET co_occurrence_count = 3 WHERE source = ? AND target = ?").run(a.id, b.id);
            const result = engine.traverse();
...
describe("GraphEngine — storeProbeResult", () => {
    ...
    it("inserts a row into probe_results table", () => {
        ...
        const outcome = JSON.parse(rows[0].outcome);
    ...
    describe("with competidor actor node seeded", () => {
        ...
        it("reinforces existing edge on repeated successful probes", () => {
            ...
            const firstEdge = db
                .prepare("SELECT * FROM edges WHERE source = ? AND target = ?")
                .get(firstId, competidorNode.id);
            const secondEdge = db
                .prepare("SELECT * FROM edges WHERE source = ? AND target = ?")
                .get(secondId, competidorNode.id);
        ...
        it("increments probe_results row count when recording multiple probes", () => {
            ...
            const count = db.prepare("SELECT COUNT(*) as cnt FROM probe_results").get().cnt;
            expect(count).toBe(2);
        });
    });
});
