...
describe("computeEconomicSignal", () => {
  ...
  it("returns positive even with refund costs when net profit > 0", () => {
    const signal = computeEconomicSignal(makeInput({ netProfit: 10000, refunds: 50000 }));
    expect(signal.direction).toBe("positive");
  });
  ...
  it("produces magnitude from expected impact comparison", () => {
    ...
    const signal = computeEconomicSignal(makeInput({ netProfit: 50000, expectedImpact: "10000" }));
  ...
  it("produces fractional magnitude when below expected", () => {
    ...
    const signal = computeEconomicSignal(makeInput({ netProfit: 30000, expectedImpact: "100000" }));
  ...
  it("computes magnitude from relative profit when no baseline or expected", () => {
    ...
    const signal = computeEconomicSignal(makeInput({ netProfit: 45000, grossRevenue: 100000 }));
  ...
  it("returns lower confidence for partial snapshots", () => {
    const complete = computeEconomicSignal(makeInput({ calculationStatus: "complete" }));
  ...
  it("clamps magnitude to [0, 1] range", () => {
    ...
    const small = computeEconomicSignal(makeInput({ netProfit: 1, grossRevenue: 100000 }));
    ...
    const large = computeEconomicSignal(makeInput({ netProfit: 900000, grossRevenue: 100000 }));
  ...
  it("returns lowest confidence for disputed snapshots", () => {
    const disputed = computeEconomicSignal(makeInput({ calculationStatus: "disputed" }));
  ...
  it("returns low confidence for unverifiable snapshots", () => {
    const unverifiable = computeEconomicSignal(makeInput({ calculationStatus: "unverifiable" }));
  ...
  it("adds refund code when refunds are present", () => {
    const signal = computeEconomicSignal(makeInput({ netProfit: 10000, refunds: 5000 }));
  ...
  it("detects exceeded baseline", () => {
    ...
    const signal = computeEconomicSignal(makeInput({ netProfit: 50000, baseline }));
  ...
  it("detects below baseline", () => {
    ...
    const signal = computeEconomicSignal(makeInput({ netProfit: 30000, baseline }));
