...
import { simulateActor, getActorPrompt, simulateCounterintelligence, COMPRADOR_PROMPT, PROVEEDOR_PROMPT, COMPETIDOR_PROMPT, } from "../../src/conversation/actorSimulator.js";
// ── getActorPrompt ──────────────────────────────────────────────────
describe("getActorPrompt", () => {
    ...
    it("throws for invalid actor type", () => {
        expect(() => getActorPrompt("unknown")).toThrow(/no válido/i);
    });
    it("error message lists valid actor types", () => {
        expect(() => getActorPrompt("unknown")).toThrow(/comprador.*proveedor.*competidor/i);
    });
});
// ── simulateActor ───────────────────────────────────────────────────
describe("simulateActor", () => {
    ...
    it("throws for invalid actor type", async () => {
        await expect(simulateActor("unknown", "test query")).rejects.toThrow(/no válido/i);
    });
    ...
    it("generates unique simulationIds for different calls", async () => {
        const r1 = await simulateActor("comprador", "precio");
        const r2 = await simulateActor("comprador", "reputación");
...
//# sourceMappingURL=actorSimulator.test.js.map