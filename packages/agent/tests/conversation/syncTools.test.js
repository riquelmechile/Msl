...
import { createSyncProductTool, createSyncAllTool, createCheckAccountTool, } from "../../src/conversation/syncTools.js";
...
function marginSyncStrategy(percentage) {
    return { type: "margin", percentage };
}
/** Create a stub sync engine that tracks calls and returns predictable results. */
function createStubSyncEngine() {
    const _syncProductCalls = [];
    const _syncAllCalls = [];
    return {
        _syncProductCalls,
        _syncAllCalls,
        async syncProduct(sourceSellerId, targetSellerId, itemId, strategies) {
        ...
        async syncAll(sourceSellerId, targetSellerId, strategies, options) {
...
describe("createSyncProductTool — unit", () => {
    ...
    it("returns error when strategies is empty", async () => {
        ...
        expect(result.error).toMatch(/no hay estrategias/i);
    });
    ...
    it("stores sync outcome in Cortex when provided", async () => {
        ...
        const outcomeMeta = JSON.parse(nodes[0].metadata);
...
//# sourceMappingURL=syncTools.test.js.map