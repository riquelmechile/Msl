...
import { createGetBusinessContextTool, createPrepareActionTool, createSimulateActorTool, createDetectProbesTool, createProposeHoneyPotTool, } from "../../src/conversation/tools.js";
...
describe("createSimulateActorTool", () => {
    ...
    it("has actorType enum with valid values", () => {
        const params = tool.parameters;
        const props = params.properties;
        const actorTypeSchema = props.actorType;
...
describe("createDetectProbesTool", () => {
    ...
    it("returns error when neither questions nor views provided", () => {
        ...
        const result = tool.execute({});
        expect(result).toHaveProperty("error");
        expect(result.error).toMatch(/requiere/i);
    });
    ...
    it("accepts both questions and views simultaneously", () => {
        ...
        const detector = (q, v) => {
            const alerts = [];
            if (q && q.length > 0)
                alerts.push(questionAlert);
            if (v && v.length > 0)
                alerts.push(viewAlert);
            return alerts;
        };
...
describe("createProposeHoneyPotTool", () => {
    ...
    function makeBlockingValidator(reason) {
        return () => ({ passed: false, reason });
    }
    it("has correct name and description", () => {
        const tool = createProposeHoneyPotTool(() => makeProposal(), makePassingValidator(), () => [makeProbeStrategy()]);
    ...
    it("returns error when strategy is not probe type", () => {
        ...
        const tool = createProposeHoneyPotTool(() => makeProposal(), makePassingValidator(), () => [marginStrategy]);
        const result = tool.execute({ strategyId: 1 });
        expect(result).toHaveProperty("error");
        expect(result.error).toMatch(/margin/);
    });
    ...
    it("returns blocked error when guardrail blocks", () => {
        const tool = createProposeHoneyPotTool(() => makeProposal(), makeBlockingValidator("No tenés estrategias de contrainteligencia activas."), () => [makeProbeStrategy()]);
        const result = tool.execute({ strategyId: 1 });
        expect(result).toHaveProperty("error");
        expect(result.error).toMatch(/contrainteligencia/);
    });
    it("calls onProposed callback when guardrail passes", () => {
        let captured = null;
        const tool = createProposeHoneyPotTool(() => makeProposal(), makePassingValidator(), () => [makeProbeStrategy()], (proposal) => { captured = proposal; });
    ...
    it("does NOT call onProposed callback when guardrail blocks", () => {
        let captured = null;
        const tool = createProposeHoneyPotTool(() => makeProposal(), makeBlockingValidator("Bloqueado."), () => [makeProbeStrategy()], (proposal) => { captured = proposal; });
        tool.execute({ strategyId: 1 });
    ...
    it("skips archived strategies", () => {
        ...
        const tool = createProposeHoneyPotTool(() => makeProposal(), makePassingValidator(), () => [archived, active]);
