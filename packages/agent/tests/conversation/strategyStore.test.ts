...
function marginRule(overrides: Partial<ParsedRule> = {}): ParsedRule {
  return {
    ruleType: "margin",
    target: "margen",
    operator: ">=",
...
function stockRule(overrides: Partial<ParsedRule> = {}): ParsedRule {
  return {
    ruleType: "stock",
    target: "stock",
    operator: "priorizar",
...
describe("strategyStore", () => {
  ...
  it("updateStrategy preserves id and updates timestamps", () => {
    ...
    const updated = store.updateStrategy(
      s.id,
      "margen 60%",
      marginRule({ value: "60%", originalText: "margen 60%" }),
    );
  ...
  it("supersedeStrategy marks old strategy and records replaced_by", () => {
    ...
    const row = db.prepare("SELECT replaced_by FROM ceo_strategies WHERE id = ?").get(old.id) as {
