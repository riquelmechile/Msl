...
describe("actorIntegration — agent loop with simulate_actor", () => {
    ...
    it("triggers actor simulation when user asks about competitor", async () => {
        ...
        const state = makeState();
        const result = await agent.converse("¿Qué haría mi competidor si bajo el precio un 10%?", state);
    ...
    it("triggers actor simulation when user asks about comprador", async () => {
        ...
        const state = makeState();
        const result = await agent.converse("¿Qué pensaría un comprador típico de este producto?", state);
        expect(result.response).toMatch(/simulación|actor/i);
    ...
    it("triggers actor simulation when user asks about proveedor", async () => {
        ...
        const state = makeState();
        const result = await agent.converse("¿Qué condiciones me daría un proveedor por 100 unidades?", state);
        expect(result.response).toMatch(/simulación|actor/i);
    ...
    it("strategyValidator blocks a proposal even after actor simulation", async () => {
        ...
        const state = makeState({
            messages: [
                ...
                {
                    role: "assistant",
                    content: "Consulté al competidor y respondió que no le molestaría si subís. " +
...
//# sourceMappingURL=actorIntegration.test.js.map