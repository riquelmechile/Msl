...
import type { DecoyProposal, Strategy } from "../../src/conversation/types.js";
...
function makeNonProbeStrategy(): Strategy {
  return {
    id: 2,
    ruleType: "margin",
...
function makeProposal(overrides: Partial<DecoyProposal> = {}): DecoyProposal {
  return {
    id: "decoy-001",
    type: "category_entry",
    description:
    ...
    tosCompliant: true,
    tosWarning:
      "⚠️ Esta operación simula actividad en MercadoLibre. Asegurate de cumplir con los Términos de Servicio.",
    ...overrides,
  };
}
...
describe("honeyPotValidator", () => {
  ...
  it("blocks when strategies array is undefined", () => {
    const result = honeyPotValidator(makeProposal(), undefined as unknown as Strategy[]);
    expect(result.passed).toBe(false);
  });
  ...
  it("returns GuardResult shape with passed and optional reason", () => {
    // Pass case
    const passResult: GuardResult = honeyPotValidator(
      makeProposal({ description: "Decoy en electrónica." }),
      [makeProbeStrategy()],
    );
    ...
    // Block case
    const blockResult: GuardResult = honeyPotValidator(makeProposal(), []);
