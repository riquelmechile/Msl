...
async function runEconomicIngestion(
  ...
  const runtime = store.__runtime ?? runStore?.__runtime ?? evidenceStore?.__runtime;
  if (runtime === undefined) {
    throw new Error("Economic pipeline test fixture requires a public memory runtime");
  }
...
function mockStore(
  ...
  // The mock insert methods don't write directly; the admitted runtime owns persistence.
...
describe("EconomicIngestionPipeline", () => {
  ...
  describe("fail-closed persistence (PR 2)", () => {
    ...
    it("2.6.5 checkpoint update throws → transaction rolls back", async () => {
      ...
      const realStore = createSqliteEconomicOutcomeStore(db);
      const realRunStore = createSqliteEconomicIngestionRunStore(db);
      // Drop the checkpoints table to force the admitted checkpoint write to fail.
