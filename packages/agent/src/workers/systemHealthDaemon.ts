...
export function runSystemHealthCheck(
  ...
  try {
    ...
    if (pendingCount > 100) {
      checks.push({ name: "bus-backlog", status: "critical", detail: `${pendingCount} pending messages` });
    } else if (pendingCount > 20) {
      checks.push({ name: "bus-backlog", status: "warning", detail: `${pendingCount} pending messages` });
    } else {
      checks.push({ name: "bus-backlog", status: "ok", detail: `${pendingCount} pending` });
    }
  } catch (e) {
    checks.push({ name: "bus-backlog", status: "warning", detail: `could not check: ${String(e)}` });
  }
  ...
  try {
    ...
    if (failed.length > 0) {
      checks.push({ name: "bus-failed", status: "warning", detail: `${failed.length} failed messages` });
    } else {
  ...
  try {
    ...
    if (nodes.length > 100000) {
      checks.push({ name: "cortex-size", status: "warning", detail: `${nodes.length}+ nodes — consider pruning` });
    } else {
      checks.push({ name: "cortex-size", status: "ok", detail: `${nodes.length}+ nodes` });
    }
  } catch (e) {
    checks.push({ name: "cortex-size", status: "warning", detail: `could not check: ${String(e)}` });
  }
