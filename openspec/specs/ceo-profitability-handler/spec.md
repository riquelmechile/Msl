...
The handler SHALL map profitability findings to Product Ads actions via `msl_prepare_product_ads_action` with `requiresApproval: true`:
...
Info-only findings (unit-economics, underinvested) SHALL produce info-report actions without requiring seller approval prompts.

#### Scenario: Margin-consuming ad triggers pause proposal
...
#### Scenario: Unit-economics finding produces info report
