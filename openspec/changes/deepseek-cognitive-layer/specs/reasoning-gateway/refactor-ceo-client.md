# Delta for deepseek-ceo-profitability-reasoning

## Purpose

Refactor `CeoDeepSeekClientImpl.reason()` to route through `DeepSeekReasoningGateway`. Preserve all existing behavior: Cortex enrichment, JSON validation, `proposalType` validation, ledger recording, and deterministic fallback. Factory still returns `null` when no API key. All existing tests in `ceoDeepSeekClient.test.ts` must pass after refactor.
