...
| Cost decomposition | 12 fixed `CostComponentType` values in `as const` tuple | Open-ended string; tagged union | Fixed taxonomy matches existing `CostSupplierEvidenceResponder` evidence categories and prevents type proliferation |
