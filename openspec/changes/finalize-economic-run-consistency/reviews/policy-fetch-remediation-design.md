# Fetch Remediation Design Gate Policy

## Target

- Operation: `review/start(target)`
- Lineage: `finalize-economic-run-consistency-fetch-remediation-design`
...
The target is the immutable native snapshot produced by `gentle-ai review-start`. It binds the current working-tree candidate, including the updated OpenSpec design, specification, policy, and task artifacts and the temporary tracked `packages/agent/src/economics/dataFetcher.ts` marker.

## Authority

The authoritative review material is the OpenSpec change `finalize-economic-run-consistency`, with emphasis on its corrected remediation `design.md`, delta specifications, policy documents, and `tasks.md`. Existing implementation is snapshot context only and is not authorized for implementation review, correction, testing, or execution in this lineage.
