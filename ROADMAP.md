...
| MCP               | **✅ 10 MCP tools** (check_account, sync_product, list_catalog, consult_cortex, list_strategies, read_sync_product_status, approve_sync_product_proposal, read_sync_product_execution_readiness, prepare_mercadolibre_write, msl_mcp_api_key). All tools run but MercadoLibre calls use stub mode without real credentials. |
...
| CI                | Format, typecheck, lint, Vitest (826 tests), Playwright E2E (7 specs), and build gates run in CI.                                                                                     |
...
| **7** | **Real ML API integration foundation**                | `@msl/mercadolibre` (MlClient with real HTTP transport + exponential backoff, OAuth manager with multi-account support, product sync engine with diff + strategy application), `@msl/mcp` (10 tools via stdio) | Real OAuth with refresh token support. HTTP transport has **exponential backoff** for 429/5xx. Sync engine diffs Plasticov → Maustian listings and applies CEO margin/stock/category/pricing strategies. All runs in stub mode without real ML credentials. | ✅ foundation                                            |
...
| **7h** | **ML credentials setup**                              | `.env.local` with `MERCADOLIBRE_CLIENT_ID`, `MERCADOLIBRE_CLIENT_SECRET`, `MSL_ENCRYPTION_KEY`                                                                                                                         | Configure real OAuth credentials for both Plasticov (source) and Maustian (target) seller accounts on MLC. OAuth flow must be tested with real tokens before any mutation call.                                                                                                                  | ❌ TODO                                                   |
...
| Protocol        | MCP for tool exposure    | 10 MCP tools (check_account, sync_product, list_catalog, consult_cortex, list_strategies, read_sync_product_status, approve_sync_product_proposal, read_sync_product_execution_readiness, prepare_mercadolibre_write, msl_mcp_api_key). All runtime-ready; MercadoLibre calls use stub mode until real credentials configured. |
